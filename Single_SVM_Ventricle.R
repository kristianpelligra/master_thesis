# Comprehensive ROC Curve Analysis for Protein-Ventricle Combinations
...
data <- read_delim("heatmap_data_norm.csv", delim = ";") %>% 
  ...
  select(2:3, 7:60, 61, 68)

# Function to analyze one protein with one ventricular volume as a combined predictor
analyze_combination <- function(data, protein_col, ventricle_col) {
  # Prepare model data
  model_data <- data %>%
    select(Subtype, !!sym(protein_col), !!sym(ventricle_col), Age, Sex)
...
# Prepare protein and ventricle columns
...
# Perform analysis for all protein-ventricle combinations
for (protein in protein_cols) {
  for (ventricle in ventricle_cols) {
    cat("Analyzing:", protein, "with", ventricle, "\n")
    results[[counter]] <- analyze_combination(data, protein, ventricle)
    counter <- counter + 1
  }
}
...
# Plot ROC curves for top combinations
pdf("top_5_roc_curves.pdf", width = 10, height = 8)
par(mfrow = c(1,1))

# Plot the best combination
best_combination_index <- which.max(sapply(results, function(x) x$auc))
plot(results[[best_combination_index]]$roc_obj, 
...
# Create legend
legend_text <- c(
  with(results[[best_combination_index]], 
       paste0(protein, " + ", ventricle, " (AUC = ", round(auc, 3), ")")),
  sapply(results[top_indices], function(x) {
    paste0(x$protein, " + ", x$ventricle, " (AUC = ", round(x$auc, 3), ")")
  })
)
